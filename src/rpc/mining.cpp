...
UniValue generateBlocks(std::shared_ptr<CReserveScript> coinbaseScript, int nGenerate, uint64_t nMaxTries, bool keepScript)
{
    ...
    if(nHeight+1 < ((uint32_t)params.GetConsensus().LTEHeight))
        throw JSONRPCError(RPC_INTERNAL_ERROR, "Couldn't excute generate now.");
    ...
    while (nHeight < nHeightEnd)
    {
        ...
        if (pblock->nHeight < (uint32_t)params.GetConsensus().LTEHeight+params.GetConsensus().LTEPremineWindow) 
		{
            // Solve sha256d.
            while (nMaxTries > 0 && (int)pblock->nNonce.GetUint64(0) < nInnerLoopCount &&
                   !CheckProofOfWork(pblock->GetPoWHash(), pblock->nBits, false, Params().GetConsensus())) {
		...
		else
		{
            ...
            CDataStream ss(SER_NETWORK, PROTOCOL_VERSION|SERIALIZE_BLOCK_LEGACY);
            ss << I;
	...
	//if(nHeight+1 == ((uint32_t)params.GetConsensus().LTEHeight+(uint32_t)params.GetConsensus().LTEPremineWindow))
      //      break;
    }
    return blockHashes;
}
...
UniValue generatetoaddress(const JSONRPCRequest& request)
{
    ...
    if(chainActive.Height()+1 >=Params().GetConsensus().LTEHeight && chainActive.Height() +1 <(Params().GetConsensus().LTEHeight+Params().GetConsensus().LTEPremineWindow))
    {
        bool valid = Params().IsPremineAddress(request.params[1].get_str());
        if (!valid)
            throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Error: Invalid address,address can not excute generate.");
    }
...
UniValue getmininginfo(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 0)
        throw std::runtime_error(
            ...
            "  \"powblockweight\": nnn, (numeric) The last block weight\n"
            "  \"currentblocktx\": nnn,     (numeric) The last block transaction\n"
            "  \"difficulty\": xxx.xxxxx    (numeric) The current difficulty\n"
            "  \"proof-of-work\": xxx.xxxxx    (numeric) The current pow difficulty\n"
            "  \"proof-of-stake\": xxx.xxxxx    (numeric) The current pos difficulty\n"
    ...
    obj.push_back(Pair("powblockweight", (uint64_t)nLastBlockWeight));
    obj.push_back(Pair("currentblocktx",   (uint64_t)nLastBlockTx));
    obj.push_back(Pair("difficulty",       (double)GetDifficulty()));
    obj.push_back(Pair("proof-of-work",  (double)GetPowDifficulty()));
    obj.push_back(Pair("proof-of-stake",  (double)GetPosDifficulty()));
...
UniValue estimatesmartfee(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() < 1 || request.params.size() > 2)
        throw std::runtime_error(
            ...
            "\nResult:\n"
            "{\n"
            "  \"feerate\" : x.x,     (numeric, optional) estimate fee-per-kilobyte (in LTE)\n"
