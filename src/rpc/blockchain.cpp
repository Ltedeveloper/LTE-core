...
UniValue blockToJSON(const CBlock& block, const CBlockIndex* blockindex, bool txDetails)
{
    ...
    if (chainActive.Contains(blockindex))
        confirmations = chainActive.Height() - blockindex->nHeight + 1;
    result.push_back(Pair("confirmations", confirmations));
    int ser_flags = (blockindex->nHeight >= Params().GetConsensus().LTEHeight) ? SERIALIZE_BLOCK_LEGACY : 0;
...
UniValue getdifficulty(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 0)
        throw std::runtime_error(
            ...
            "\nResult:\n"
            "n.nnn       (numeric) the last block difficulty as a multiple of the minimum difficulty.\n"
            "\nExamples:\n"
...
UniValue getblockheader(const JSONRPCRequest& request)
{
    ...
    if (!fVerbose)
    {
        int ser_flags = (chainActive.Height() >= Params().GetConsensus().LTEHeight) ? SERIALIZE_BLOCK_LEGACY : 0;
...
{ //  category              name                      actor (function)         okSafe argNames
  //  --------------------- ------------------------  -----------------------  ------ ----------
    ...
    { "blockchain",         "getpowdifficulty",          &getpowdifficulty,          true,  {} },
    { "blockchain",         "getposdifficulty",          &getposdifficulty,          true,  {} },
