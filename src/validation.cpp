...
static bool IsUAHFenabled(const Consensus::Params& consensusparams, int nHeight) {
    return nHeight >= consensusparams.LTEHeight;
}
                              ...
                              bool fOverrideMempoolLimit, const CAmount& nAbsurdFee, std::vector<COutPoint>& coins_to_uncache)
{
...

    {
        ...
        {
            LogPrint(BCLog::MEMPOOL, "replacing tx %s with %s for %s LTE additional fees, %d delta bytes\n",
...
bool ReadBlockFromDisk(CBlock& block, const CDiskBlockPos& pos, const Consensus::Params& consensusParams)
{
	...
	if (!block.IsProofOfStake()) {
	    ...
	    if (!postfork && !CheckProofOfWork(block.GetPoWHash(), block.nBits, postfork, consensusParams))
			...
			return error("ReadBlockFromDisk: Errors in Equip block header at %s", pos.ToString());
    }
...
bool IsInitialBlockDownload()
{
    ...
    if (chainActive.Tip()->nChainWork < UintToArith256(chainParams.GetConsensus().nMinimumChainWork))
        return true;
    /*if (fSkipHardforkIBD && chainActive.Tip()->nHeight + 1 >= (int)chainParams.GetConsensus().LTEHeight)
        return false;
...
int32_t ComputeBlockVersion(const CBlockIndex* pindexPrev, const Consensus::Params& params, uint32_t miningType)
{
    ...
    if(pindexPrev!=nullptr && pindexPrev->nHeight+1 >= Params().GetConsensus().LTEHeight)
...
bool CheckBlock(const CBlock& block, CValidationState& state, const Consensus::Params& consensusParams, bool fCheckPOW, bool fCheckMerkleRoot)
{
    ...
    if(block.nHeight >= Params().GetConsensus().LTEHeight) {
...
static bool ContextualCheckBlock(const CBlock& block, CValidationState& state, const Consensus::Params& consensusParams, const CBlockIndex* pindexPrev)
{
    ...
    /*if (nHeight >= consensusParams.LTEHeight&&
        nHeight < consensusParams.LTEHeight+ consensusParams.LTEPremineWindow)
    {
